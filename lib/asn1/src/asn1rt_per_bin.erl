...
getbits_as_binary(0,Buffer) ->
    {{0,<<>>},Buffer};
getbits_as_binary(Num,{0,Bin}) when Num > 16, is_bitstring(Bin) ->
    Used = Num rem 8,
    Pad = (8 - Used) rem 8,
%    Nbytes = Num div 8,
    case Bin of
    <<Bits:Num,_:Pad,RestBin/binary>> ->
...
%% then pick one byte.
getbits_as_list(N,{0,Bin},Acc) when N >= 8, is_bitstring(Bin) ->
    case Bin of
    <<B7:1,B6:1,B5:1,B4:1,B3:1,B2:1,B1:1,B0:1,Rest/binary>> ->
        getbits_as_list(N-8,{0,Rest},[B0,B1,B2,B3,B4,B5,B6,B7|Acc]);
    _ -> throw({error, incomplete})
    end;
getbits_as_list(N,{Used,Bin},Acc) when N >= 4, Used =< 4, is_bitstring(Bin) ->
    NewUsed = Used + 4,
    Rem = 8 - NewUsed,
    case Bin of
    <<_:Used,B3:1,B2:1,B1:1,B0:1,_:Rem, Rest/binary>> ->
        NewRest = case Rem of 0 -> Rest; _ -> Bin end,
        getbits_as_list(N-4,{NewUsed rem 8,NewRest},[B0,B1,B2,B3|Acc]);
    _ -> throw({error, incomplete})
    end;
getbits_as_list(N,{Used,Bin},Acc) when N >= 2, Used =< 6, is_bitstring(Bin) ->
    NewUsed = Used + 2,
    Rem = 8 - NewUsed,
    case Bin of
    <<_:Used,B1:1,B0:1,_:Rem, Rest/binary>> ->
        NewRest = case Rem of 0 -> Rest; _ -> Bin end,
        getbits_as_list(N-2,{NewUsed rem 8,NewRest},[B0,B1|Acc]);
    _ -> throw({error, incomplete})
    end;
getbits_as_list(N,{Used,Bin},Acc) when Used =< 7, is_bitstring(Bin) ->
    NewUsed = Used + 1,
    Rem = 8 - NewUsed,
    case Bin of
    <<_:Used,B0:1,_:Rem, Rest/binary>> ->
        NewRest = case Rem of 0 -> Rest; _ -> Bin end,
...
getbit({0,Buffer = <<B:1,_:7,_/binary>>}) ->
    {B,{1,Buffer}};
getbit({Used,Buffer}) when is_bitstring(Buffer)  ->
    Unused = (8 - Used) - 1,
    case Buffer of
    <<_:Used,B:1,_:Unused,_/binary>> ->
        {B,{Used+1,Buffer}};
    _ -> throw({error, incomplete})
    end;
...
getbits({0,Buffer},Num) when (Num rem 8) == 0, is_bitstring(Buffer) ->
    case Buffer of
    <<Bits:Num,Rest/binary>> ->
        {Bits,{0,Rest}};
    _ -> throw({error, incomplete})
    end;
getbits({Used,Bin},Num) when is_bitstring(Bin) ->
	...
	_ ->
	    ...
	    case Bin of
	    <<_:Used,Bits:Num,_UBits:Unused,_/binary>> ->
...
getoctets({0,Buffer},Num) when is_bitstring(Buffer) ->
    case Buffer of
    <<Val:Num/integer-unit:8,RestBin/binary>> ->
        {Val,{0,RestBin}};
    _ -> throw({error, incomplete})
    end;
...
getoctets_as_bin({0,Bin},Num) when is_bitstring(Bin) ->
    case Bin of
    <<Octets:Num/binary,RestBin/binary>> ->
        {Octets,{0,RestBin}};
    _ -> throw({error, incomplete})
    end;
getoctets_as_bin({_U,Bin},Num) when is_bitstring(Bin) ->
    case Bin of
    <<_Padding,Octets:Num/binary,RestBin/binary>> ->
        {Octets,{0,RestBin}};
    _ -> throw({error, incomplete})
    end;
...
decode_fragmented_bits(<<0:1,Len:7,Bin/binary>>,C,Acc) ->
	...
	case (Len rem 8) of
	    0 ->
		case Bin of
		<<Value:Len/binary-unit:1,Bin2/binary>> ->
		...
		U = 8 - Rem,
		case Bin of
		<<Value:Bytes/binary-unit:8,Bits1:Rem,Bits2:U,Bin2/binary>> ->
...
decode_fragmented_octets(<<0:1,Len:7,Bin/binary>>,C,Acc) ->
    case Bin of
    <<Value:Len/binary-unit:8,Bin2/binary>> ->
